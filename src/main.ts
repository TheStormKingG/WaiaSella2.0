...
function renderInventoryCategories() {
  ...
  // Add "Manage Categories" button - app icon style
  const manageBtn = h('div', {
    class: 'category-app-btn',
    style: 'aspect-ratio: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 16px; background: #FFFFFF; border-radius: 20px; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: transform 0.2s ease, box-shadow 0.2s ease; position: relative; overflow: hidden;'
  })
  manageBtn.innerHTML = `
    <div style="width: 56px; height: 56px; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px;">
    ...
    <div style="font-size: 11px; font-weight: 600; color: #1f2937; text-align: center; line-height: 1.3;">Manage<br/>Categories</div>
  `
  manageBtn.addEventListener('mouseenter', () => { 
    manageBtn.style.transform = 'scale(1.05)'
    manageBtn.style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)'
  })
  manageBtn.addEventListener('mouseleave', () => { 
    manageBtn.style.transform = 'scale(1)'
    manageBtn.style.boxShadow = '0 2px 8px rgba(0,0,0,0.1)'
  })
  ...
  // Add category buttons - modern app icon style
